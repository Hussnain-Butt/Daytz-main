...
const UserProfileCard: React.FC<UserProfileCardProps> = ({ userId }) => {
  // Get auth context directly inside the component
  ...
  const fetchProfileDataAsync = useCallback(async () => {
    if (!userId || !authUser) {
      console.log('UserProfileCard: Aborting fetch. Missing userId or authenticated user.');
      setProfile(null);
      setError(null);
    ...
    console.log(`UserProfileCard: Initiating fetch for userId: ${userId}`);
    ...
    try {
      ...
      console.log(`UserProfileCard: API response for ${userId}. Status: ${response.status}.`);

      if (response.data && response.data.userId) {
        setProfile(response.data);
      } else {
        const message = response.status === 404 ? `User profile not found.` : `Profile data is incomplete.`;
        setError(message);
        setProfile(null);
      }
    } catch (err: any) {
      ...
      console.error(`UserProfileCard fetch error for ${userId}:`, errorMessage, err);
  ...
  return (
    <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <View style={styles.userInfoContainer}>
        <Image source={profile.profilePictureUrl ? { uri: profile.profilePictureUrl } : DEFAULT_AVATAR_ASSET} style={styles.avatar} onError={(e) => console.warn(`Avatar load error: ${e.nativeEvent.error}`)} />
        <Text style={styles.userName}>{`${profile.firstName || ''} ${profile.lastName || ''}`.trim() || 'User Name'}</Text>
          ...
          <>
            ...
            <View style={styles.videoSectionContainer}>
              ...
              <View style={styles.videoPlayerWrapper}>
                {isLoadingPlayableUrl && (<View style={styles.videoPlaceholder}><PaperActivityIndicator size="large" color={colors.GoldPrimary || '#FFD700'} /><Text style={styles.videoStatusText}>Loading Video...</Text></View>)}
                {!isLoadingPlayableUrl && playableBioVideoUrl && !videoPlaybackError && (
                  <Video ref={videoPlayerRef} source={{ uri: playableBioVideoUrl }} style={styles.bioVideoPlayer} useNativeControls resizeMode={ResizeMode.CONTAIN} isLooping={false} onError={(errorMsg: string) => { setVideoPlaybackError(`Could not play video.`); setPlayableBioVideoUrl(null); }} onLoad={(status) => { if ((status as AVPlaybackStatusSuccess).isLoaded) { setVideoPlaybackError(null); } }} onPlaybackStatusUpdate={(status) => { if ((status as AVPlaybackStatusError).error && !videoPlaybackError) { setVideoPlaybackError(`Playback issue.`); } }} />
                )}
                {!isLoadingPlayableUrl && (!playableBioVideoUrl || videoPlaybackError) && (
                  <View style={styles.videoPlaceholder}>
                    <IconButton icon={videoPlaybackError ? 'alert-circle-outline' : 'video-off-outline'} size={48} iconColor={colors.LightGrey || '#A0A0A0'} />
                    <Text style={styles.videoStatusText}>{videoPlaybackError ? videoPlaybackError : 'No bio video uploaded.'}</Text>
                    {videoPlaybackError && <Button mode="text" onPress={fetchPlayableVideoUrlLogic} textColor={colors.GoldPrimary || '#FFD700'} style={{ marginTop: 10 }} labelStyle={{ fontSize: 14 }} compact>Try Again</Button>}
                  </View>
                )}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  container: { flex: 1, backgroundColor: colors.GreyDarker || '#181818', minHeight: screenHeight * 0.4 },
  centered: { justifyContent: 'center', alignItems: 'center', padding: 20 },
  userInfoContainer: { width: '100%', paddingVertical: 20, paddingHorizontal: 15, alignItems: 'center' },
  avatar: { width: 100, height: 100, borderRadius: 50, marginBottom: 12, borderWidth: 2, borderColor: colors.GoldPrimary || '#FFD700', backgroundColor: colors.GreyMedium || '#333' },
  userName: { fontSize: 22, fontWeight: '600', color: colors.White || '#FFF', textAlign: 'center' },
  userEmail: { fontSize: 14, color: colors.LightGrey || '#AAA', marginBottom: 15, textAlign: 'center' },
  divider: { marginVertical: 15, height: 1, backgroundColor: colors.Grey || '#424242', width: '95%', alignSelf: 'center' },
  infoSection: { width: '100%', marginBottom: 10, paddingHorizontal: 5 },
  sectionTitle: { fontSize: 18, fontWeight: '500', color: colors.GoldPrimary || '#FFD700', marginBottom: 12 },
  infoRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, minHeight: 30 },
  ...
  errorText: { fontSize: 16, color: colors.PinkPrimary || '#FF6B6B', textAlign: 'center', fontWeight: '500', marginBottom: 15 },
  retryButton: { marginTop: 10, backgroundColor: colors.GoldPrimary || '#FFD700', paddingVertical: 8, paddingHorizontal: 20, borderRadius: 20 },
  retryButtonText: { color: colors.Black || '#000', fontWeight: 'bold', fontSize: 14 },
  videoSectionContainer: { width: '100%', alignItems: 'flex-start', marginBottom: 10, paddingHorizontal: 5 },
  videoPlayerWrapper: { width: '100%', aspectRatio: 16 / 9, backgroundColor: colors.Black || '#000000', borderRadius: 10, overflow: 'hidden', justifyContent: 'center', alignItems: 'center', marginTop: 5 },
  bioVideoPlayer: { width: '100%', height: '100%' },
  videoPlaceholder: { width: '100%', height: '100%', justifyContent: 'center', alignItems: 'center', padding: 15 },
  videoStatusText: { color: colors.LightGrey || '#A0A0A0', textAlign: 'center', marginTop: 10, fontSize: 14, lineHeight: 20 },
});
